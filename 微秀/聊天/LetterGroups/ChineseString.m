...
//    for (NSString* object in tempArray)
    ...
    {
        NSString *pinyin = [((ChineseString*)object[kTitleKey]).pinYin substringToIndex:1];
        //不同
...
//    for (NSString* object in tempArray)
    for (NSDictionary* object in tempArray){
         ...
         NSString *pinyin = [((ChineseString*)object[kTitleKey]).pinYin substringToIndex:1];
         NSString *string = ((ChineseString*)object[kTitleKey]).string;
        //不同
        if(![tempString isEqualToString:pinyin])
        {
            //分组
            item = [NSMutableArray array];
            NSDictionary *dic1=@{kImgKey:object[kImgKey],kTitleKey:string};
            [item  addObject:dic1];
        ...
        {
            NSDictionary *dic1=@{kImgKey:object[kImgKey],kTitleKey:string};
            [item  addObject:dic1];//由1.可知，item但指针指向了LetterResult，所以再只执行[item  addObject:string]时，也隐含执行[LetterResult addObject:item]//1.
        }
    }
    return LetterResult;
}
...
+(NSMutableArray*)ReturnSortChineseArrar:(NSArray*)stringArr
{
    ...
    for(int i=0;i<[stringArr count];i++)
    {
        ...
        NSDictionary *dic;
        if ([stringArr[i] isKindOfClass:[NSArray class]]) {
            
            NSArray *arr=stringArr[i];
            dic=arr[0];
        }
        if ([stringArr[i] isKindOfClass:[NSDictionary class]]) {
            dic=stringArr[i];
        }
        
        
        //chineseString.string=[NSString stringWithString:[stringArr objectAtIndex:i]];
        chineseString.string=dic[kTitleKey];
       ...
       // NSLog(@"string====%@",chineseString.string);
        ...
        NSDictionary *dic1=@{kImgKey:dic[kImgKey],kTitleKey:chineseString};
//        [chineseStringsArray addObject:chineseString];
        [chineseStringsArray addObject:dic1];
